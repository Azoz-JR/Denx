...
-(void)initializeIslamicCalendar{
    ...
    [btnNext setTitle:Lang(@"Next") forState:UIControlStateNormal];
    ...
    [btnPrevious setTitle:Lang(@"Previous") forState:UIControlStateNormal];
    ...
    dateBGColor=[UIColor whiteColor];
...
-(void)configureCellLabels:(UILabel*)titelLbl detailLabel:(UILabel *)detailLbl atIndexPath:(NSIndexPath*)indexPath{
    ...
    if (indexPath.item<7) {
        ...
        if (indexPath.item>=startIndex && datesArry.count>indexPath.item-startIndex) {
            ...
            detailLbl.text=[self getGregorianDayFromDate:date];
...
- (void)setUpIndexs:(NSInteger)num{
    ...
    if (datesArry.count > 0) {
        ...
        NSDate *firstDate = [datesArry objectAtIndex:15];
        NSDateFormatter *dateFormatter = [[NSDateFormatter alloc] init];
        [dateFormatter setDateFormat:@"yyyy-MM-dd-HH:mm:ss"];
//        NSLog(@"setUpIndexs firstDate %@", [dateFormatter stringFromDate:firstDate]);
...
//        NSLog(@"setUpIndexs IslamicUmmAlQura %@", lblMonth.text);
...
-(NSString*)getGregorianDayFromDate:(NSDate*)date{
    ...
    NSLocale *gbLocale = [[NSLocale alloc] initWithLocaleIdentifier:[[LanguageManager shareInstance] getHttpLanguageType]];
//        NSLocale *gbLocale = [[NSLocale alloc] initWithLocaleIdentifier:@"en"];
    [formatter setLocale:gbLocale];
    retStr = [formatter stringFromNumber:someNumber];
    
//    if ([[[LanguageManager shareInstance] getHttpLanguageType]  isEqual: @"ar"]) {
//        NSDecimalNumber *someNumber = [NSDecimalNumber decimalNumberWithString:retStr];
//        NSNumberFormatter *formatter = [[NSNumberFormatter alloc] init];
...
-(BOOL)todayDayName:(NSIndexPath *)indexPath{
    ...
    if ([currDayName isEqualToString:Lang(@"Mon")] && indexPath.item==0) {
        
        return YES;
    }else if ([currDayName isEqualToString:Lang(@"Tue")] && indexPath.item==1) {
        
        return YES;
    }else if ([currDayName isEqualToString:Lang(@"Wed")] && indexPath.item==2) {
        
        return YES;
    }else if ([currDayName isEqualToString:Lang(@"Thu")] && indexPath.item==3) {
        
        return YES;
    }else if ([currDayName isEqualToString:Lang(@"Fri")] && indexPath.item==4) {
        
        return YES;
    }else if ([currDayName isEqualToString:Lang(@"Sat")] && indexPath.item==5) {
        
        return YES;
    }else if ([currDayName isEqualToString:Lang(@"Sun")] && indexPath.item==6) {
