...
@interface SedentarySetViewController ()<UITableViewDelegate,UITableViewDataSource,BasePickerViewDelegate,MWWeekViewControllerDelegate>
...
#pragma mark Data
/// 模型
@property(nonatomic, strong) SedentarySetModel *model;
/// 列表数据源
...
@implementation SedentarySetViewController
...
- (void)getSedentary {
    WEAKSELF
    [DHBleCommand getSedentary:^(int code, id  _Nonnull data) {
        if (code == 0) {
            [weakSelf saveModel:data];
        } else {
            weakSelf.model = [SedentarySetModel currentModel];
        }
        [weakSelf initData];
        [weakSelf setupUI];
...
- (void)saveModel:(id)data {
    DHSedentarySetModel *model = data;
...
- (void)addObservers {
    ...
    //状态更新
    [DHNotificationCenter addObserver:self selector:@selector(sedentaryChange) name:BluetoothNotificationSedentaryChange object:nil];
}

- (void)sedentaryChange {
    self.model = [SedentarySetModel currentModel];
    ...
    for (int i = 0; i < self.dataArray.count; i++) {
        ...
        } else if ([cellModel.leftTitle isEqualToString:Lang(@"str_repeat")]) {
            cellModel.subTitle = [self repeatsTitle:self.repeats];
...
- (void)navRightButtonClick:(UIButton *)sender {
    ...
    model.repeats = self.repeats;
    
    SHOWINDETERMINATE
    WEAKSELF
    [DHBleCommand setSedentary:model block:^(int code, id  _Nonnull data) {
        if (code == 0) {
...
- (void)saveModel {
    ...
    self.model.repeats = [self.repeats transToJsonString];
...
#pragma mark - BaseWeekViewControllerDelegate

- (void)repeatsUpdate:(NSArray *)repeats {
    self.repeats = repeats;
    MWBaseCellModel *cellModel = self.dataArray[2];
    cellModel.subTitle = [self repeatsTitle:repeats];
    
    [self performSelector:@selector(delayReloadRows) withObject:nil afterDelay:0.2];
    
}

- (void)delayReloadRows {
...
- (NSArray *)subTitles {
    if (!_subTitles) {
        _subTitles = @[@"",
                       ...
                       [self repeatsTitle:self.repeats],
