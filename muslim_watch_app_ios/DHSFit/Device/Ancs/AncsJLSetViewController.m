//
//  AncsJLSetViewController.m
//  DHSFit
//
//  Created by DHS on 2023/11/15.
//

#import "AncsJLSetViewController.h"


@interface AncsJLSetViewController ()<UITableViewDelegate,UITableViewDataSource>
...
@implementation AncsJLSetViewController
...
- (void)saveModel:(id)data {
    ...
    ancsModel.isJLSinaWeiBo = model.isJLSinaWeiBo;
    ancsModel.isJLBand = model.isJLBand;
    ancsModel.isJLTelegram = model.isJLTelegram;
    ancsModel.isJLBetween = model.isJLBetween;
    ancsModel.isJLNavercafe = model.isJLNavercafe;
    ancsModel.isJLNetflix = model.isJLNetflix;
...
- (void)navRightButtonClick:(UIButton *)sender {
    ...
    model.isJLSinaWeiBo = self.isJLSinaWeiBo;
    model.isJLBand = self.isJLBand;
    model.isJLTelegram = self.isJLTelegram;
    model.isJLBetween = self.isJLBetween;
    model.isJLNavercafe = self.isJLNavercafe;
    model.isJLNetflix = self.isJLNetflix;
...
- (void)saveModel {
    ...
    self.model.isJLSinaWeiBo = self.isJLSinaWeiBo;
    self.model.isJLBand = self.isJLBand;
    self.model.isJLTelegram = self.isJLTelegram;
    self.model.isJLBetween = self.isJLBetween;
    self.model.isJLNavercafe = self.isJLNavercafe;
    self.model.isJLNetflix = self.isJLNetflix;
...
- (void)initData {
    ...
    self.isJLSinaWeiBo = self.model.isJLSinaWeiBo;
    self.isJLBand = self.model.isJLBand;
    self.isJLTelegram = self.model.isJLTelegram;
    self.isJLBetween = self.model.isJLBetween;
    self.isJLNavercafe = self.model.isJLNavercafe;
    self.isJLNetflix = self.model.isJLNetflix;
    
    self.isOther = self.model.isOther;
    
    self.isAll = (self.isCall &&
                  self.isSMS &&
                  self.isQQ &&
                  self.isWechat &&
                  self.isWhatsapp &&
    ...
    for (int i = 0; i < self.titles.count; i++) {
        ...
        if (i == 0) {
            ...
            cellModel.isOpen = self.isJLSinaWeiBo;
        } else if (i == 14) {
            cellModel.isOpen = self.isQQ;
        } else if (i == 15) {
            cellModel.isOpen = self.isWechat;
        } else if (i == 16) {
            cellModel.isOpen = self.isJLBand;
        } else if (i == 17) {
            cellModel.isOpen = self.isJLTelegram;
        } else if (i == 18) {
            cellModel.isOpen = self.isJLBetween;
        } else if (i == 19) {
            cellModel.isOpen = self.isJLNavercafe;
        } else if (i == 20) {
            cellModel.isOpen = self.isYoutube;
        } else if (i == 21) {
            cellModel.isOpen = self.isJLNetflix;
        } else if (i == 22) {
...
- (void)switchViewValueChanged:(UISwitch *)sender {
    for (int i = 0; i < self.dataArray.count; i++) {
        ...
        if (sender.tag - 100 == i && i == 0) {
           ...
           self.isJLSinaWeiBo = sender.isOn;
           cellModel.isOpen = sender.isOn;
    ...
    if (sender.tag == 100) {
        ...
        self.isJLSinaWeiBo = self.isAll;
        self.isJLBand = self.isAll;
        self.isJLTelegram = self.isAll;
        self.isJLBetween = self.isAll;
        self.isJLNavercafe = self.isAll;
        self.isJLNetflix = self.isAll;
...
- (NSArray *)images {
    ...
    if (!_images) {
        _images = @[@"device_ancs_all",
                    ...
                    @"device_ancs_sinaweibo",
                    @"device_ancs_qq",
                    @"device_ancs_wechat",
                    @"device_ancs_band",
                    @"device_ancs_telegram",
                    @"device_ancs_between",
                    @"device_ancs_navercafe",
                    @"device_ancs_youtube",
                    @"device_ancs_netflix",
                    @"device_ancs_other"];
    }
