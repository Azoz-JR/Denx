...
@interface HeartRateSetViewController ()<UITableViewDelegate,UITableViewDataSource,BasePickerViewDelegate>
...
#pragma mark Data
/// 模型
@property(nonatomic, strong) HeartRateSetModel *model;
/// 列表数据源
...
- (void)viewDidLoad {
    ...
    [self getHeartRateMode];
    
    
}

- (void)getHeartRateMode {
    WEAKSELF
    [DHBleCommand getHeartRateMode:^(int code, id  _Nonnull data) {
        if (code == 0) {
            [weakSelf saveModel:data];
        } else {
            weakSelf.model = [HeartRateSetModel currentModel];
        }
        [weakSelf initData];
        [weakSelf setupUI];
...
- (void)addObservers {
    ...
    //状态更新
    [DHNotificationCenter addObserver:self selector:@selector(heartRateModeChange) name:BluetoothNotificationHeartRateModeChange object:nil];
}

- (void)heartRateModeChange {
    self.model = [HeartRateSetModel currentModel];
    ...
    for (int i = 0; i < self.dataArray.count; i++) {
        ...
        if ([cellModel.leftTitle isEqualToString:Lang(@"str_hr_watcher")]) {
            cellModel.isOpen = self.isOpen;
        } else if ([cellModel.leftTitle isEqualToString:Lang(@"str_hr_rate")]) {
