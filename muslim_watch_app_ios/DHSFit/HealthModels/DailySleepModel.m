//
//  DailySleepModel.m
//  DHSFit
//
//  Created by DHS on 2022/6/6.
//

#import "DailySleepModel.h"

@implementation DailySleepModel

-(instancetype)init{
    if (self = [super init]) {
        self.userId = DHUserId;
        self.macAddr = DHMacAddr;
        self.isUpload = NO;
        
        self.date = @"";
        self.timestamp = @"";
        
        self.deep = 0;
        self.light = 0;
        self.wake = 0;
        self.total = 0;
        self.wakeCount = 0;
        self.beginTime = @"";
        self.endTime = @"";
        self.items = @"";
        
    }
    return self;
}

+ (__kindof DailySleepModel *)currentModel:(NSString *)date {
    DailySleepModel *model =[DailySleepModel findFirstWithFormat:@"WHERE userId = '%@' AND date = '%@'",DHUserId,date];
    if (!model) {
        model = [[DailySleepModel alloc] init];
        model.date = date;
        model.timestamp = [NSDate timeStampFromtimeStr:[NSString stringWithFormat:@"%@000000",date]];
    }
    return model;
}

+ (NSArray *)queryModels:(NSString *)startDateStr endDateStr:(NSString *)endDateStr {
    NSDate *startDate = [startDateStr dateByStringFormat:@"yyyyMMdd"];
    NSDate *endDate = [endDateStr dateByStringFormat:@"yyyyMMdd"];

    NSArray *models =[DailySleepModel findWithFormat:@"WHERE userId = '%@' and date >= '%@' and date <= '%@'",DHUserId, startDateStr, endDateStr];

    NSTimeInterval time = [endDate timeIntervalSinceDate:startDate];
    NSInteger count = time / (24*60*60) + 1;
    
    NSMutableArray *sleepModels = [NSMutableArray array];
    NSMutableArray *dateStrs = [NSMutableArray array];
    for (int i = 0; i < count; i++) {
        DailySleepModel *model = [[DailySleepModel alloc] init];
        NSDate *date = [startDate dateAfterDay:i];
        model.date = [date dateToStringFormat:@"yyyyMMdd"];
        [sleepModels addObject:model];
        [dateStrs addObject:model.date];
    }
    
    for (DailySleepModel *model in models) {
        NSInteger index = [dateStrs indexOfObject:model.date];
        [sleepModels replaceObjectAtIndex:index withObject:model];
    }
    return sleepModels;
}

+ (NSArray *)queryYearModels:(NSDate *)date {
    NSString *startDate;
    NSString *endDate;
    NSInteger dayCount = 31;
    BOOL isLeapYear = (((date.year%4 == 0)&&(date.year%100 != 0))||(date.year%400 == 0));
    NSMutableArray *valueArray = [NSMutableArray array];
    for (int i = 1; i < 13; i++) {
        if (i == 2) {
            dayCount = isLeapYear ? 29 : 28;
        } else if (i == 4 ||
            i == 6 ||
            i == 9 ||
            i == 11) {
            dayCount = 30;
        }
        startDate = [NSString stringWithFormat:@"%04ld%02ld%02ld",(long)date.year,(long)i,(long)1];
        endDate = [NSString stringWithFormat:@"%04ld%02ld%02ld",(long)date.year,(long)i,(long)dayCount];
        NSArray *models = [DailySleepModel findWithFormat:@"WHERE userId = '%@' and date >= '%@' and date <= '%@'",DHUserId, startDate, endDate];
        NSInteger sum = 0;
        for (DailySleepModel *model in models) {
            if (model.total) {
                NSInteger total = model.deep+model.light;
                sum += total;
            }
        }
        [valueArray addObject:@(sum)];
    }
    return valueArray;
}

+ (NSArray *)queryUploadModels {
    NSArray *models = [DailySleepModel findWithFormat:@"WHERE userId = '%@' AND isUpload = 0",DHUserId];
    return models;
}

+ (NSArray *)queryVisitorModels {
    NSArray *models = [DailySleepModel findWithFormat:@"WHERE userId = '%@'",DHVisitorId];
    return models;
}

+ (DailySleepModel *)queryModel:(NSString *)timestamp {
    DailySleepModel *model =[DailySleepModel findFirstWithFormat:@"WHERE userId = '%@' and timestamp = '%@'",DHUserId,timestamp];
    return model;
}

+ (NSString *)getTableName{
    return @"t_sync_sleep";
}

@end
